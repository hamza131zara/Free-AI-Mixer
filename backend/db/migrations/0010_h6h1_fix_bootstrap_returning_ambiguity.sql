...
create or replace function public.free_ai_mixer_bootstrap_account_workspace(
  p_app_user_id uuid,
...
begin
  ...
  if not found then
    insert into public.workspace_memberships as inserted_membership (
      workspace_id, user_id, role, status
    ) values (
      p_personal_workspace_id, p_app_user_id, 'owner', 'active'
    )
    on conflict do nothing
    returning inserted_membership.workspace_id
      into v_inserted_membership_workspace_id;
