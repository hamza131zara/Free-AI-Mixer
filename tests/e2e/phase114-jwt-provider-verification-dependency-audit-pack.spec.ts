...
test.describe("phase114 jwt provider verification dependency audit pack", () => {
  test("jwt verification dependency is not installed or imported yet", async () => {
    ...
    expect(authSource).not.toContain("JWKS");
    expect(authSource).not.toContain("JWK");
  });

  test("jwt provider remains fail-closed and route enforcement remains deferred", async () => {
  ...
  test("dependency audit keeps secrets frontend storage and artifact delivery blocked", async () => {
    const routeSource = readSource("backend/routes/exports.ts");
