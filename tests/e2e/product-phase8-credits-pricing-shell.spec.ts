...
test.describe("product phase 8 credits and pricing shell", () => {
  test("credits page renders honest planned policy without fake balance", async ({ page }) => {
    ...
    await expect(page.getByText("platform_credits_not_configured")).toBeVisible();
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          subscriptionBoundary: {
            ...
            message: "Subscriptions are not configured. No premium entitlement is active.",
          },
 ...
 await expect(
  page.getByTestId("free-paid-policy-grid").getByText(
