...
const specPath = path.join(
  ...
  "tests",
  "e2e",
  "phase71-worker-manual-drain-runtime-pack.spec.ts",
);
...
const createTempRoot = async (): Promise<string> =>
  fs.mkdtemp(path.join(os.tmpdir(), "phase71-worker-"));
...
const createRecord = (
  ...
  jobId: "job-phase71-default",
  requestId: "request-phase71-default",
  timelineId: "timeline-phase71-default",
...
class TrackingRuntimeDrainRegistry implements ExportJobRegistry {
  ...
  async create(_input: CreateExportJobInput): Promise<BackendExportJobRecord> {
    throw new Error("create should not be called in phase71 runtime drain pack");
  }
  ...
  async getByRequestId(): Promise<BackendExportJobRecord | undefined> {
    throw new Error(
      "getByRequestId should not be called in phase71 runtime drain pack",
    );
  }
  ...
  async transition(
    ...
    throw new Error("transition should not be called in phase71 runtime drain pack");
  }
}
...
const createWorkerAdapter = async (
  ...
  await fs.writeFile(
    ...
    Buffer.from("phase71-video-bytes"),
  );
  return { ok: true };
};

test.describe("phase71 worker manual drain runtime pack", () => {
  test("manual drain runtime helper uses injected/backend dependencies directly without startup or loop activation", async () => {
    await withEnv(
      {
      ...
      async () => {
        ...
        try {
          ...
          const trackingRegistry = new TrackingRuntimeDrainRegistry([
            createRecord({
              jobId: "job-phase71-success",
              requestId: "request-phase71-success",
            }),
          ]);
          ...
          const result = await drainBackendWorkerOnce(manualDeps, {
            workerId: "worker-phase71-manual",
          });

          expect(result.workerId).toBe("worker-phase71-manual");
          expect(result.attemptedJobIds).toEqual(["job-phase71-success"]);
          ...
          expect(trackingRegistry.lifecycleCalls).toEqual([
            "claim:job-phase71-success",
            "markRendering:job-phase71-success",
            "markFinalizing:job-phase71-success",
            "markSuccess:job-phase71-success",
          ]);

          const successRecord = await trackingRegistry.getById("job-phase71-success");
  ...
  test("source keeps manual drain helper separate from startup, loop, routes, app/server rollout, and unsupported runtime behavior", async () => {
    const [
      specSource,
    ...
    expect(backendDependenciesSource).toContain("export const drainBackendWorkerOnce = async");
    expect(backendDependenciesSource).toContain(
      "dependencies: BackendDependencies = createBackendDependencies()",
    );
    expect(backendDependenciesSource).toContain("drainRenderWorkerOnce(");
    ...
    expect(appSource).not.toContain("drainBackendWorkerOnce(");
    expect(serverSource).toContain("createApp()");
    expect(serverSource).not.toContain("drainBackendWorkerOnce(");
