...
test.describe("phase56 supabase worker claim lease boundary", () => {
  ...
  test("source proves worker claim and lease behavior remain deferred until schema and runtime support exist", async () => {
    const [
      specSource,
      registrySource,
    ...
    expect(schemaSource).not.toContain("claimed_by_worker_id");
    expect(schemaSource).not.toContain("claim_expires_at");
